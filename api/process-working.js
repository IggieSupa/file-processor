// Working API for Supabase Storage file processing
...
function cleanData(row, headers) {
  ...
  headers.forEach((header, index) => {
    ...
    if (key.includes("ind_delivery") || key === "dead_dying_indicator" || key === "he_outlet" || key === "merchandising_call" || key === "bees_registered" || key === "draught_outlet" || key === "fridge_outlet") {
      if (typeof value === "string") {
...
async function createJSONBatches(rows, headers, jobId, batchSize = 1000) {
  ...
  for (let i = 0; i < totalBatches; i++) {
    ...
    // Update progress
    const progress = Math.round(((i + 1) / totalBatches) * 50);
    jobs.set(jobId, {
      ...jobs.get(jobId),
      currentBatch: i + 1,
      progress,
...
async function handler(req, res) {
  ...
  try {
    ...
    if (fileExtension === "xlsx") {
      ...
      return res.status(400).json({
        error: "Invalid file type. Please upload XLSX files only.",
        jobId,
      });
    }
